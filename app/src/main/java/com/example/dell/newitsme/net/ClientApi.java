...
public class ClientApi {
    ...
    private Map _oauthParam = new HashMap();
    ...
    public void signUpByEmail(String email , String pass , String nick, ApiListener listener){
        ...
        _oauthParam.clear();//全局Map  ,很多接口共用一个map，所以要清除
        
        _oauthParam.put(PLATFORM, EMAIL);// private Map _oauthParam = new HashMap();
        _oauthParam.put(PASSWORD, pass);
        _oauthParam.put(ACCOUNT, email);
        _oauthParam.put(NICK ,nick);
        callCmd_POST(REGISTER, _oauthParam, listener);//key的值为REGISTER 根据key的值取链接" url": "http://54.169.250.64/user/account/register.php"
    }
    ...
    public  void loginByEmail(String pass, String email, final ApiListener listener) {
        ...
        pass = Encrypt.SHA1(Encrypt.SHA1(pass) + "flybird");//加密
        _oauthParam.put(PLATFORM, EMAIL);
        _oauthParam.put(TOKEN, pass);
        _oauthParam.put(OPENID, email);

        callCmd_POST(USER_LOGIN, _oauthParam, new ApiListener() {
            @Override
